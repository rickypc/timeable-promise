...
// Copy files over.
await Promise.all(
  ['LICENSE', 'README.md'].map((file) => copyFile(join(base, file), join(path, file))),
);

// Generate package.json.
const pkg = require(join(base, 'package.json'));
['devDependencies', 'imports', 'scripts'].forEach((key) => {
...
pkg.exports = Object.fromEntries(
  Object.entries(entry)
    .map<Export>(([name]) => {
      ...
      return [key, { import: `./${name}.js`, require: `./${name}.cjs`, types: `./${name}.d.ts` }];
    })
...
await writeFile(
  ...
  JSON.stringify(
    Object.fromEntries(Object.entries(pkg).sort(([a], [b]) => a.localeCompare(b))),
    null,
    2,
  ),
);
